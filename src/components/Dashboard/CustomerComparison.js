...
function CustomerComparison(props) {
    ...
    React.useEffect(() => {
      if(dashboard_reports){
          let val = [];
          let dt = [];
          dashboard_reports.loansbyindustry.forEach(v => val.push(Number(v.total)));
          dashboard_reports.loansbyindustry.forEach(v => (v.sector === null) ? 'Other' : dt.push(v.sector));
          setValue(val);
          setData(dt);
      }
    }, [dashboard_reports]);
